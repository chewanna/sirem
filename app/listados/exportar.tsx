...
export const ExportarButtons: React.FC<ExportarButtonsProps> = ({ listado }) => {
                    ...
                    <td style="vertical-align: top; padding: 10px; border-bottom: 1px solid #000;">
                        <table class="inner-table" style="width: 100%; border: none; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 9.5px; line-height: 1.4;">
                            <tr>
                                ...
                                <td style="text-align: right; white-space: nowrap;">Cumple Edad Límite: <span style="text-transform: uppercase;">${getEdadLimite(p.fecha_nacimiento)}</span></td>
                            </tr>
                            <tr>
                                <td><span class="strong">F. NAC.:</span> ${formatDate(p.fecha_nacimiento)}</td>
                                <td><span class="strong">EDAD:</span> ${getTimeDiff(p.fecha_nacimiento)}</td>
                                <td><span class="strong">F. EMP.:</span> ${formatDate(p.fecha_empleo)}</td>
                                <td><span class="strong">ANT. GDO.:</span> ${getTimeDiff(p.fecha_grado)}</td>
                            </tr>

                            <tr>
                                <td><span class="strong">F. ALTA.:</span> ${formatDate(p.fecha_ingreso)}</td>
                                <td><span class="strong">TPO. SERV.:</span> ${getTimeDiff(p.fecha_ingreso)}</td>
